function [p_alloc, eta, obj, status] = ldt_qt_cvx_pac_rc(...
    ...
    cvx_begin quiet
        ...
        for k = 1:K
            ...
            for i = 1:K_c
                ...
                %% BD interference to NOMA user
                bd_interf_noma = 0;
                for l = 1:K
                    bd_interf_noma = bd_interf_noma + h_c_tot(k,i,l)*eta_var(l);
                end
                ...
                %% NOMA transformed objective
                noma_obj = noma_obj ...
                    + log2(1 + beta(k,i)) - beta(k,i) ...
                    + 2*t(k,i)*sqrt((1+beta(k,i))*h_ki(k,i))*sqrt(p(k,i)) ...
                    - t(k,i)^2 * ApB;

                %% BD objective only for strong user
                if i == strong_user

                    bd_interf_BD = 0;
                    for l = 1:K
                        if l ~= k
                            bd_interf_BD = bd_interf_BD + h_c_int(k,l)*eta_var(l);
                        end
                    end
        ...
        for k = 1:K
            ...
            %% Eta box constraint
            0 <= eta_var(k) <= 1;
            eta_var(k) >= 0.4; % TEMP for testing
            ...
            for i = 1:K_c
                ...
                %% BD QoS and EH only for strong user
                if i == strong_user
    ...
    fprintf('NOMA surrogate obj = %.6f\n', double(noma_obj));
    fprintf('BD surrogate obj   = %.6f\n', double(bd_obj));
